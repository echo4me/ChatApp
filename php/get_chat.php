    ...
    if(isset($_SESSION['unique_id'])){
        ...
        if(mysqli_num_rows($query) > 0){
            while($row = mysqli_fetch_assoc($query)){
                if($row['outgoing_id'] === $outgoing_id){// if they equil this mean is msg sender
                    $output .= '
                ...
                }else{// mean he is reicever
                    $output .= '
                                <div class="chat incoming"><img src="php/images/'.$row['image'].'" alt="logo">
                                <div class="details">
                                    <p>'.$row['message'].'</p>
                                </div>
                                </div> ';
